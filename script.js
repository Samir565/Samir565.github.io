...
$("#year").textContent=new Date().getFullYear();$("#menu").onclick=()=>$("#nav").classList.toggle("open");$$("header nav a").forEach(a=>a.onclick=()=>$("#nav").classList.remove("open"));
const words=["Hardware Design","Machine Learning","Signal Processing","Smart Energy Systems"];let w=0,c=0,d=false;
function type(){let x=words[w];$("#typed").textContent=x.slice(0,c);if(!d&&c<x.length)c++;else if(d&&c>0)c--;else if(!d){d=true;return setTimeout(type,1200)}else{d=false;w=(w+1)%words.length}setTimeout(type,d?35:70)}type();
